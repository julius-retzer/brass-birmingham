 ...
 *
 * Fixture: ?demo=beerchoice — canal round 10, George to act with £13, three of
 * his own breweries in reach AND a Warrington merchant barrel (£5 bonus). The
 * engine's old auto-pick would have drained a brewery and left the £5 on the
 * table, so this journey is the whole point of the feature.
 */
...
test('the beer picker offers every legal source, and the merchant barrel pays its bonus', async ({
  ...
  await expect(sources).toHaveCount(4) // 3 own breweries + the merchant barrel
  await expect(
  ...
  await expect(
    page.locator('g[data-city="coalbrookdale"] rect[stroke-dasharray="6 4"]'),
  ).toBeVisible()
