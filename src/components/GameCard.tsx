...
export function GameCard({ card, isSelected = false, onClick, disabled = false }: GameCardProps) {
  ...
  const getCardTitle = () => {
    switch (card.type) {
      ...
      case 'industry':
        return card.industries.map(i => i.charAt(0).toUpperCase() + i.slice(1)).join(' / ');
      case 'wild_location':
        ...
        {card.type === 'location' && (
          <div className="text-sm">
            <span className={cn(
              ...
              card.color === 'blue' ? 'bg-blue-200 dark:bg-blue-800' :
              card.color === 'teal' ? 'bg-teal-200 dark:bg-teal-800' :
