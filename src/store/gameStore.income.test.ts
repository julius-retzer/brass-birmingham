...
const setup = (playerCount = 2) => {
  ...
  const players = Array.from({ length: playerCount }, (_, i) => ({
    ...
    character: ['Richard Arkwright', 'Eliza Tinsley', 'Isambard Kingdom Brunel', 'George Stephenson'][i] as any,
  }))
  ...
  for (let p = 0; p < playerCount; p++) {
    for (let a = 0; a < actionsPerPlayer; a++) {
      ...
      if (currentPlayer.hand.length > 0) {
        ...
        snapshot = actor.getSnapshot()
        const cardId = snapshot.context.players[snapshot.context.currentPlayerIndex]!.hand[0]!.id
...
describe('Game Store - Income Collection', () => {
  describe('Positive Income Collection', () => {
    test('collects income at end of round', () => {
      ...
      // Set player incomes
      actor.send({ type: 'TEST_SET_PLAYER_STATE', playerId: 0, income: 10, money: 0 })
      actor.send({ type: 'TEST_SET_PLAYER_STATE', playerId: 1, income: 15, money: 0 })
      ...
      // Check logs
      expect(snapshot.context.logs.some(l => l.message.includes('P1 collected £10 income'))).toBe(true)
      expect(snapshot.context.logs.some(l => l.message.includes('P2 collected £15 income'))).toBe(true)
    })
    ...
    test('handles different income levels correctly', () => {
      ...
      for (const testCase of testCases) {
        actor.send({ type: 'TEST_SET_PLAYER_STATE', playerId: 0, income: testCase.income, money: 0 })
    ...
    test('does not collect income on final round of era', () => {
      ...
      actor.send({ type: 'TEST_SET_PLAYER_STATE', playerId: 0, income: 10, money: 0 })
      actor.send({ type: 'TEST_SET_PLAYER_STATE', playerId: 1, income: 10, money: 0 })
  ...
  describe('Negative Income Scenarios', () => {
    test('player can afford to pay negative income', () => {
      ...
      actor.send({ type: 'TEST_SET_PLAYER_STATE', playerId: 0, income: -5, money: 20 })
      ...
      // Check log
      expect(snapshot.context.logs.some(l => l.message.includes('paid £5 negative income'))).toBe(true)
    })
    ...
    test('player cannot afford negative income - sells industry tiles', () => {
      ...
      // Check logs
      expect(snapshot.context.logs.some(l => l.message.includes('sold coal industry for £9'))).toBe(true)
    })
    ...
    test('multiple industries sold to cover shortfall', () => {
      ...
      const saleLogs = snapshot.context.logs.filter(l => l.message.includes('sold') && l.message.includes('industry'))
    ...
    test('loses VP when cannot cover shortfall even after selling all industries', () => {
      ...
      // Check logs
      expect(snapshot.context.logs.some(l => l.message.includes('lost') && l.message.includes('VP'))).toBe(true)
    })
  })
    ...
    test('calculates sale value correctly (half cost rounded down)', () => {
      ...
      for (const { cost, expectedSaleValue } of testCases) {
        ...
        if (expectedSaleValue > 0) {
          expect(snapshot.context.logs.some(l => 
  ...
  describe('Edge Cases', () => {
    test('handles income at minimum negative level (-10)', () => {
      ...
      actor.send({ type: 'TEST_SET_PLAYER_STATE', playerId: 0, income: -10, money: 20 })
      ...
      expect(snapshot.context.logs.some(l => l.message.includes('paid £10 negative income'))).toBe(true)
    })
    ...
    test('turn order affects income collection sequence', () => {
      ...
      actor.send({ type: 'TEST_SET_PLAYER_STATE', playerId: 0, income: 5, money: 0 })
      actor.send({ type: 'TEST_SET_PLAYER_STATE', playerId: 1, income: 10, money: 0 })
      actor.send({ type: 'TEST_SET_PLAYER_STATE', playerId: 2, income: 15, money: 0 })
      ...
      expect(snapshot.context.logs.filter(l => l.message.includes('collected £')).length).toBe(3)
    })
  })
})