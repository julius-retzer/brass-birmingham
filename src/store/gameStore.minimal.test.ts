...
describe('Minimal XState Test', () => {
  ...
  test('can create test players', async () => {
    const { getInitialPlayerIndustryTiles } = await import('../data/industryTiles')
  ...
  test('can test location cards creation logic step by step', async () => {
    ...
    const count = playerCount === 2 ? sampleLocation.twoPlayers : playerCount === 3 ? sampleLocation.threePlayers : sampleLocation.fourPlayers
  ...
  test('can access locations data directly', async () => {
    ...
    const hasCircularRef = content.includes('locations') && content.includes('industries')
