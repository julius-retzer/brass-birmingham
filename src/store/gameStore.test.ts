...
test('network action - canal era', () => {
  ...
  snapshot = actor.getSnapshot();
  expect(snapshot.value).toEqual({ playing: { performingAction: { networking: 'selectingCard' } } });
  ...
  // Verify card selection
  expect(snapshot.value).toEqual({ playing: { performingAction: { networking: 'selectingLink' } } });
  ...
  expect(snapshot.value).toEqual({ playing: { performingAction: { networking: 'confirmingLink' } } });
  ...
  expect(lastLog?.type).toBe('action');
  expect(lastLog?.message).toContain('Player 1');
  expect(lastLog?.message).toContain('built a canal link between birmingham and dudley');
});

test('network action - rail era', () => {
  const { actor } = setupTestGame();
  let snapshot = actor.getSnapshot();

  // Set up rail era
  snapshot.context.era = 'rail';
  const initialPlayer = snapshot.context.players[0];
  assert(initialPlayer, 'Expected player 1 to exist');
  const initialHand = [...initialPlayer.hand];
  const initialDiscardPile = [...snapshot.context.discardPile];
  const initialMoney = initialPlayer.money;
  const initialResources = { ...snapshot.context.resources };
  ...
  // Verify money was spent (£5 for first rail link)
  expect(finalPlayer.money).toBe(initialMoney - 5);

  // Verify coal was consumed in rail era
  verifyGameState(snapshot, {
    resources: {
      ...initialResources,
      coal: initialResources.coal - 1
    }
  });
  ...
  expect(lastLog?.type).toBe('action');
  expect(lastLog?.message).toContain('Player 1');
  expect(lastLog?.message).toContain('built a rail link between birmingham and dudley');
});
