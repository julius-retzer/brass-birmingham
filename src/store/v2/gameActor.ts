...
export interface Player {
  ...
  character:
    ...
    | 'Robert Owen'
    | 'Henry Bessemer'
  money: number // PUBLIC: all players can see money
  victoryPoints: number // PUBLIC: all players can see victory points  
  income: number // PUBLIC: all players can see income
  
  // PRIVATE PLAYER STATE - should only be visible to the owning player
  hand: Card[] // PRIVATE: only this player should see their hand
  // Industry tiles on player mat (available to build) - now with quantities
  industryTilesOnMat: Record<IndustryType, IndustryTileWithQuantity[]> // PRIVATE: only this player should see available tiles
  
  // PUBLIC BOARD STATE - built items visible to all players
  links: Link[] // PUBLIC: all players can see built links
  industries: { // PUBLIC: all players can see built industries
    location: CityId
    type: IndustryType
    level: number
    flipped: boolean
...
export interface GameState {
  // PUBLIC GAME STATE - visible to all players, should be synchronized
  players: Player[] // MIXED: contains both public and private player data
  currentPlayerIndex: number // PUBLIC: all players can see whose turn it is
  era: 'canal' | 'rail' // PUBLIC: all players can see current era
  round: number // PUBLIC: all players can see current round
  actionsRemaining: number // PUBLIC: all players can see remaining actions
  
  // PUBLIC RESOURCE STATE - visible to all players
  resources: { // PUBLIC: shared resource pools
    coal: number
    iron: number
    beer: number
  }
  // Resource markets - explicit structure with max capacity per price
  coalMarket: Array<{ price: number; cubes: number; maxCubes: number }>// PUBLIC: market state visible to all
  ironMarket: Array<{ price: number; cubes: number; maxCubes: number }> // PUBLIC: market state visible to all
  logs: LogEntry[] // PUBLIC: game log visible to all players
  
  // PUBLIC CARD STATE - deck information visible to all
  drawPile: Card[] // PUBLIC: deck state (but not card order) visible to all
  discardPile: Card[] // PUBLIC: discard pile visible to all
  wildLocationPile: WildLocationCard[] // PUBLIC: wild card piles visible to all
  wildIndustryPile: WildIndustryCard[] // PUBLIC: wild card piles visible to all
  
  // UI STATE - should be client-only, never synchronized to server
  selectedCard: Card | null // UI: current player's card selection
  selectedCardsForScout: Card[] // UI: current player's scout selections
  selectedLink: { // UI: current player's link selection
    from: CityId
    to: CityId
  } | null
  selectedSecondLink: { // UI: current player's second link selection
    from: CityId
    to: CityId
  } | null
  selectedLocation: CityId | null // UI: current player's location selection
  selectedIndustryTile: IndustryTile | null // UI: current player's tile selection
  selectedTilesForDevelop: IndustryType[] // UI: current player's develop selections
  
  // PUBLIC TURN STATE - visible to all players
  spentMoney: number // PUBLIC: spending in current round (affects turn order)
  playerSpending: Record<string, number> // PUBLIC: track spending per player per round
  turnOrder: string[] // PUBLIC: player IDs in turn order (updated each round based on spending)
  isFinalRound: boolean // PUBLIC: all players should know if it's the final round
  
  // PUBLIC GAME SYSTEMS - visible to all players
  merchants: Merchant[] // PUBLIC: merchant locations and bonuses visible to all
  
  // UI ERROR STATE - client-only error handling
  lastError: string | null // UI: error messages for current player
  errorContext: 'build' | 'network' | 'develop' | 'sell' | 'scout' | null // UI: error context
}
...
export type GameActor = typeof gameActor
export type GameActorSnapshot = StateFrom<typeof gameActor>
export type GameActorSend = Actor<typeof gameActor>['send']
export type GameActorActor = Actor<typeof gameActor>
              ...
              states: {
                selectingCard: {
                  on: {
                    ...
                    CANCEL: {
                      target: '#brassGameActor.playing.action.selectingAction',
                ...
                confirmingBuild: {
                  on: {
                    CONFIRM: {
                      target: '#brassGameActor.playing.actionComplete',
